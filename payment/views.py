...
def checkout(request):
    ...
    discount_amount = (total_with_delivery * Decimal(discount_percentage)) / 100
...
def billing_info(request):
    ...
    if request.method == 'POST':
        ...
        request.session['my_shipping'] = my_shipping  # تخزين البيانات في الجلسة
...
def process_order(request):
    ...
    if request.method == 'POST':
        ...
        discount_percentage = request.session.get('discount_percentage', 0)
        discount_amount = (total_with_delivery * Decimal(discount_percentage)) / 100
        amount_paid = total_with_delivery - discount_amount
